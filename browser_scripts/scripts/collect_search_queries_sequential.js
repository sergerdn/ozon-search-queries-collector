(async () => {
  ...
  async function sequentialPaginateRequests(keyword_query, maxRetries) {
    ...
    try {
      ...
      // Step 2: Generate all offset-based requests (sequential)
      const totalRequests = Math.ceil(totalItems / paginationLimit);
      console.log(`[Info] Starting ${totalRequests} sequential requests.`);

      // Step 3: Execute each request sequentially
      for (let i = 0; i < totalRequests; i++) {
        ...
        try {
          // Execute the request
          const data = await fetchWithRetry(requestBody);
          capturedData.push(...data.data); // Add data to capturedData
        } catch (error) {
          console.error("[Error] Failed request in sequential execution:", error);
          // Stop processing further requests on failure
          throw new Error("Stopping further requests due to a failure.");
        }
      }
    } catch (error) {
  ...
  try {
    const capturedData = await sequentialPaginateRequests(keywordQuery, maxRetries);
